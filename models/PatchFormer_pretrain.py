...
class Model(nn.Module): 
    ...
    def _unpatching(self, x):
        ...
        return folded
    def random_masking(self, x, mask_rate=0.5):
        """
        Randomly mask a portion of the input tensor x along the patch_num dimension.
        Args:
            x: Input tensor of shape (batch_size, patch_num, patch_len * enc_in).
            mask_rate: Proportion of patches to mask.
        Returns:
            masked_x: Tensor with masked patches.
            mask: Binary mask indicating which patches were masked.
        """
        batch_size, patch_num, _ = x.size()
        num_patches_to_mask = int(patch_num * mask_rate)
        mask = torch.zeros(batch_size, patch_num).to(x.device)
        for i in range(batch_size):
            indices = torch.randperm(patch_num)[:num_patches_to_mask]
            mask[i, indices] = 1
        mask = mask.unsqueeze(-1)  # Expand mask to match x's dimensions
        masked_x = x * (1 - mask)
