...
class TaggedDatasetWrapper(Dataset):
    def __init__(self, base_dataset: Dataset, dataset_id: str):
    ...
    def __getitem__(self, idx):
        x, y, x_time, y_time = self.base_dataset[idx]
        return x, y, x_time, y_time, self.dataset_id
...
def data_provider(args, flag):
    ...
    for dataset_id, path in args.dataset_paths.items():
        data_set = Data(
            path = path,
            split = flag,
        ...
        wrapped_dataset = TaggedDatasetWrapper(data_set, dataset_id)
        train_dataloaders[dataset_id] = DataLoader(
                                                    wrapped_dataset,
                                                    batch_size=batch_size,
                                                    shuffle=shuffle_flag,
                                                    num_workers=args.num_workers,
                                                    drop_last=drop_last)
        feature_dims[dataset_id] = data_set.num_features

    return wrapped_dataset, train_dataloaders, feature_dims
