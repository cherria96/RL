...
class TimeSeriesDataset(Dataset):
    ...
    def prepare_data(self):
        ...
        df_all = pd.concat(dfs, axis=0).sort_values('Date').reset_index(drop=True)
        ...
        df_split = df_all.loc[split_indices]
        data_columns = df_split.columns.difference(['Date'])
        data = df_split[data_columns].values

        if self.scale:
            train_data = df_all.loc[indices[:train_end], data_columns].values
    ...
    def __getitem__(self, index):
        ...
        nan_mask = torch.isnan(x).any(dim=0)  # shape: [features]
        x = x[:, ~nan_mask]  # keep only non-NaN columns
        y = y[:, ~nan_mask]  # optional: match y to x (if same target columns)
